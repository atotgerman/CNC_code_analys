...
function initFileUpload(){
  ...
  input.onchange=() => {
    if(input.files.length>0){
      globalThis.console.log("CHANGE FIRED");
      const file=input.files["0"];
      globalThis.console.log("FILE OK");
...
function loadCncFiles(){
  StartImmediate(Delay(() => Bind_1(GetCncFilesRpc(), (a) => {
    cncFilesVar().Set(a);
    return Zero();
  })), null);
}
...
function computeDirections(lines){
  return choose((_1) => {
    let _2;
    const a=_1[0];
    const b=_1[1];
...
function cncFilesVar(){
  return _c.cncFilesVar;
}
function nameVar(){
  return _c.nameVar;
}
...
let _c=Lazy((_i) => class $StartupCode_Client {
  ...
  static gcodeVar;
  static cncFilesVar;
  static selectedCncVar;
  static zoomVar;
  static {
    this.zoomVar=_c_1.Create_1(1);
    this.selectedCncVar=_c_1.Create_1("");
      ...
      if(value[1].$==1){
        if(value[1].$1.$==1){
          if(value[1].$1.$1.$==1){
            if(value[1].$1.$1.$1.$==0){
              const s_1=value[0];
              _9=Dynamic_1(attrName, Map3((_10, _11, _12) => s_1+_10+_11+_12, value[1].$0, value[1].$1.$0, value[1].$1.$1.$0));
            }
            else {
              const s_2=value[0];
              _9=Dynamic_1(attrName, Map((vs) => s_2+concat_2("", vs), Sequence(value[1])));
            }
          }
          else {
            const s_3=value[0];
            _9=Dynamic_1(attrName, Map2((_10, _11) => s_3+_10+_11, value[1].$0, value[1].$1.$0));
          }
        }
        else {
          const t=value[0];
          if(t!=null&&t.length===0)_9=Dynamic_1(attrName, value[1].$0);
          else {
            const s_4=value[0];
            _9=Dynamic_1(attrName, Map((v) => s_4+v, value[1].$0));
          }
        }
      }
...
function GetCncFilesRpc(){
  return Bind_1((new AjaxRemotingProvider()).Async("Server/GetCncFilesRpc", []), (o) => Return(((DecodeList(Id()))())(o)));
}
